<template>
    <div class="hello_box">
        <div class="top_nav_box">
            <div class="top_nav">
                <h1 class="title_ds" @click="goBack()" >毕设小说</h1>
                <ul class="type_ul">
                    <li>
                ...
                <div class="search_box">
                    <input type="text" class="search_input" placeholder="书名,作者搜索" v-model="obj.searchStr" />
                    <button class="search_button" @click="searchBook()">🔍</button>
                </div>
              <div class="manage">
                <span @click="goadminlogin"> 后台管理</span>
              </div>
...
let reload = async () => {
  ...
  try {
    let res = await axios.get("http://localhost:8080/user/info");
    if (res.data.code === 0) {
  ...
  // 以下代码是固定的类型数组，不需要从服务器获取
  obj.types = ["都市", "科幻", "历史", "悬疑", "武侠", "玄幻", "言情"];
};
onMounted(() => {
    reload();
});
