...
let getdicuss= async () => {
  try {
    const res = await axios.get("http://localhost:8080/novel/comment");
    if (res.data.code === 0) {
      comments.value = res.data.data
      // 获取回复
      // const res2 = await axios.get("http://localhost:8080/novel/reply");
      // if (res.data.code === 0) {
      //   comments2.value = comments.value.concat(res2.data.data)
...
// const getdicuss = async () => {
//   try {
//     const res = await axios.get("http://localhost:8080/novel/comment");
//     if (res.data.code === 0) {
//       let comments1 = res.data.data.map(comment => ({ ...comment, source: 'comments1' }));
//
//       // 获取回复
//       const res2 = await axios.get("http://localhost:8080/novel/reply");
//       if (res2.data.code === 0) {
//         let comments2 = res2.data.data.map(reply => ({ ...reply, source: 'comments2' }));
//
//         // 合并数据
//         comments.value = comments1.concat(comments2);
//         console.log('Combined Comments:', comments.value); // 调试日志
//         ElMessage.success('获取成功！');
//       } else {
//         ElMessage.warning('请稍后再试！');
//       }
...
let handleDelete = async (id) => {
  try {
    const res = await axios.delete(`http://localhost:8080/novel/comment/${id}`);
    if (res.data.code === 0) {
